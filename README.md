...
## Other observations
I'm not sure if the adversarial agent was acting with a plan, but the pharmacy assistant sometimes got confused when the adversary repeated the messages from the assistant and / or pretended to be himself to be an assitant, almost in an attempt to reverse roles in the interaction ([example](examples/long_automated_chat.json)). I could've added an explicit phrase in the prompt or an example but since it was not breaking any guidelines and it didn't feel like a behaviour it was likely to encounter in a real world situation, I haven't done so.
